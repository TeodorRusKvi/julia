...
function simulate_queue(arrivals::Vector{Float64}, service_times::Vector{Float64}, num_servers::Int, max_capacity::Int, max_queue_length::Int, time_limit::Float64, verbose::Bool=false)
    queue_state = init_queue_state(num_servers, max_capacity, max_queue_length)
    events, total_planned_time = generate_events(arrivals, service_times, num_servers, false)
    for event in events
